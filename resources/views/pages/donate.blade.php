...
@section('content')
@php
$selectedCause = request()->get('cause', 'general');
$defaultAmount = is_numeric(request()->get('amount')) ? intval(request()->get('amount')) : 1000;
@endphp
                    ...
                    <div class="card p-4 p-md-5 border-0 shadow-sm rounded-4" style="background: #ffffff;">
                        ...
                        <div class="row g-3 mb-4" id="amount-pills">
                            <div class="col-sm-4 col-6">
                                <button type="button" class="btn w-100 py-3 rounded-3 amt-pill {{ $defaultAmount == 500 ? 'btn-primary active text-white' : 'btn-outline-secondary' }}" data-amt="500" style="{{ $defaultAmount == 500 ? 'background: var(--ul-primary); border-color: var(--ul-primary);' : '' }}">
                                    <h4 class="mb-1 font-bold">₹500</h4>
                                    <small style="font-size: 11px;">1 Month School Books</small>
                                </button>
                            </div>
                            <div class="col-sm-4 col-6">
                                <button type="button" class="btn w-100 py-3 rounded-3 amt-pill {{ $defaultAmount == 1000 ? 'btn-primary active text-white' : 'btn-outline-secondary' }}" data-amt="1000" style="{{ $defaultAmount == 1000 ? 'background: var(--ul-primary); border-color: var(--ul-primary);' : '' }}">
                                    <h4 class="mb-1 font-bold">₹1,000</h4>
                                    <small style="font-size: 11px;">1 Child Skill &amp; Bag Kit</small>
                                </button>
                            </div>
                            <div class="col-sm-4 col-6">
                                <button type="button" class="btn w-100 py-3 rounded-3 amt-pill {{ $defaultAmount == 2500 ? 'btn-primary active text-white' : 'btn-outline-secondary' }}" data-amt="2500" style="{{ $defaultAmount == 2500 ? 'background: var(--ul-primary); border-color: var(--ul-primary);' : '' }}">
                                    <h4 class="mb-1 font-bold">₹2,500</h4>
                                    <small style="font-size: 11px;">Health Kit for 5 Families</small>
                                </button>
                            </div>
                            <div class="col-sm-4 col-6">
                                <button type="button" class="btn w-100 py-3 rounded-3 amt-pill {{ $defaultAmount == 5000 ? 'btn-primary active text-white' : 'btn-outline-secondary' }}" data-amt="5000" style="{{ $defaultAmount == 5000 ? 'background: var(--ul-primary); border-color: var(--ul-primary);' : '' }}">
                                    <h4 class="mb-1 font-bold">₹5,000</h4>
                                    <small style="font-size: 11px;">1 Month Tailoring Course</small>
                                </button>
                            </div>
                            <div class="col-sm-4 col-6">
                                <button type="button" class="btn w-100 py-3 rounded-3 amt-pill {{ $defaultAmount == 10000 ? 'btn-primary active text-white' : 'btn-outline-secondary' }}" data-amt="10000" style="{{ $defaultAmount == 10000 ? 'background: var(--ul-primary); border-color: var(--ul-primary);' : '' }}">
                                    <h4 class="mb-1 font-bold">₹10,000</h4>
                                    <small style="font-size: 11px;">Adopt a Village Learning Hub</small>
                                </button>
                            </div>
                            <div class="col-sm-4 col-6">
                                <button type="button" class="btn w-100 py-3 rounded-3 amt-pill {{ $defaultAmount == 25000 ? 'btn-primary active text-white' : 'btn-outline-secondary' }}" data-amt="25000" style="{{ $defaultAmount == 25000 ? 'background: var(--ul-primary); border-color: var(--ul-primary);' : '' }}">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group input-group-lg">
                                ...
                                <input type="number" id="custom-amount-input" class="form-control" value="{{ $defaultAmount }}" min="100" placeholder="e.g. 1500">
                            </div>
                        </div>
                        ...
                        <!-- Donor Checkout Form -->
                        <form action="{{ route('donate.process') }}" method="POST" id="donorCheckoutForm">
                            @csrf
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-12">
                                    ...
                                    <select name="merchant_param2" class="form-select">
                                        <option value="general" {{ $selectedCause == 'general' ? 'selected' : '' }}>General Humanitarian Fund</option>
                                        <option value="education" {{ $selectedCause == 'education' ? 'selected' : '' }}>Child &amp; Girl Education Drive</option>
                                        <option value="healthcare" {{ $selectedCause == 'healthcare' ? 'selected' : '' }}>Rural Free Healthcare &amp; Eye Camps</option>
                                        <option value="women" {{ $selectedCause == 'women' ? 'selected' : '' }}>Women Vocational Sewing &amp; Micro-Enterprise</option>
                                        <option value="meals" {{ $selectedCause == 'meals' ? 'selected' : '' }}>Daily Hot Meals &amp; Ration Distribution</option>
                                    </select>
                                </div>
                            </div>

                            <input type="hidden" name="amount" id="final-form-amount" value="{{ $defaultAmount }}">

                            <div class="mt-4">
                                <button type="submit" class="ul-btn w-100 justify-content-center py-3 fs-5">
                                    <i class="flaticon-fast-forward-double-right-arrows-symbol"></i> Proceed to Pay ₹<span id="btn-amt-display">{{ number_format($defaultAmount) }}</span>
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card p-4 border-0 shadow-sm rounded-4 mb-4 text-center" style="background: #ffffff;">
                        ...
                        <div class="p-3 bg-light rounded-4 d-inline-block mx-auto mb-3" style="max-width: 240px;">
                            <img src="{{ asset('images/scanner.jpeg') }}" alt="UPI QR Code" class="img-fluid rounded-3">
                        </div>
